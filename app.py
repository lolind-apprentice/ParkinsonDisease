...
if app_mode == "About the App":
    ...
    with tab_cls_stat:
        if df_cls is not None:
            ...
            # Extract patient IDs from name column (e.g. phon_R01_S01_1 -> S01)
...
# --- PAGE 2: CLASSIFICATION & MODEL AUDITING ---
    ...
    if df is not None:
        # Extract patient ID string and add it to dataframe
        ...
        # TAB 3: PARTIAL DEPENDENCE PLOTS (NEW)
        with audit_tab3:
            st.write("### 📈 Partial Dependence Plots (PDP)")
            st.markdown(
                "Partial Dependence Plots illustrate the marginal effect of top acoustic features on the "
        ...
        # TAB 4: PATIENT-LEVEL METRICS AUDIT (NEW)
        with audit_tab4:
            st.write("### 👤 Patient-Level Performance Audit")
            st.markdown(
                "Because multiple voice recordings come from the same individual, evaluating performance "
                "aggregated at the **Patient/Subject Level** ensures the model is accurate per patient rather than "
                "just over-indexing on individual voice samples."
            )
            ...
            # Majority Vote Diagnosis per Patient
        ...
        # TAB 6: Misclassifications Audit
        with audit_tab6:
